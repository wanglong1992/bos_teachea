...
import cn.itcast.bos.domain.chart.ChartData;
...
//		headRow.createCell(6).setCellValue("收件人地址");
	...
	public String exportPdf() throws IOException, DocumentException {
		...
		Font font = new Font(cn, 10, Font.NORMAL, Color.RED);
...
//		String jrxml = ServletActionContext.getServletContext().getRealPath("/WEB-INF/jasper/waybill.jrxml");
	...
	// 组织highcharts的数据
	@Action(value="report_exportHighcharts",results={@Result(name="success",type="json")})
	public String exportHighcharts(){
		List<Object[]> list = wayBillService.exportHighcharts();
		// 封装数据
		List<ChartData> chartList = new ArrayList<>();
		for(int i=0;i<list.size();i++){
			ChartData chartData = new ChartData();
			chartData.setData((Long)list.get(i)[0]);
			chartData.setName(list.get(i)[1].toString());
			chartList.add(chartData);
		}
		pushValueStack(chartList);
		return SUCCESS;
	}
	...
	//方案二
	@Action("reportSubArea_exportPdf")
	public String exportSubAreaPdf() throws Exception {
			// 查询出 满足当前条件 结果数据
		//	List<SubArea> subAreas = subAreaService.findSubArea();

			// 下载导出
			// 设置头信息
			ServletActionContext.getResponse().setContentType("application/pdf");
			String filename = "分区分布jasper.pdf";
			String agent = ServletActionContext.getRequest().getHeader("user-agent");
			filename = FileUtils.encodeDownloadFilename(filename, agent);
			ServletActionContext.getResponse().setHeader("Content-Disposition","attachment;filename=" + filename);
			...
			// 根据 jasperReport模板 生成pdf
			// 读取jasper模板文件
			String jrxml = ServletActionContext.getServletContext().getRealPath("/WEB-INF/jasper/subArea.jrxml");
			JasperReport report = JasperCompileManager.compileReport(jrxml);
			...
			Map<String, Object> paramerters = new HashMap<String, Object>();
			paramerters.put("company", "传智播客-黑马程序员");
			paramerters.put("projectDirector", "项目总监:叶思帅");
			// Field变量
...
/*		// 查询出 满足当前条件 结果数据
		...
		// 写PDF数据
		// 向document 生成pdf表格
		Table table = new Table(9);//创建7列的表格
		...
		// 写表头
		table.addCell(buildCell("分拣编号", font));
		table.addCell(buildCell("省", font));
		table.addCell(buildCell("市", font));
		table.addCell(buildCell("区", font));
		table.addCell(buildCell("关键字", font));
		table.addCell(buildCell("起始号", font));
		table.addCell(buildCell("终止号", font));
		table.addCell(buildCell("单双号", font));
		table.addCell(buildCell("辅助关键字", font));
		// 写数据
		for (SubArea subArea : subAreas) {
			table.addCell(buildCell(subArea.getId(), font));
			table.addCell(buildCell(subArea.getArea().getProvince(), font));
			table.addCell(buildCell(subArea.getArea().getCity(), font));
			table.addCell(buildCell(subArea.getArea().getDistrict(), font));
			table.addCell(buildCell(subArea.getKeyWords(), font));
			table.addCell(buildCell(subArea.getStartNum(), font));
			table.addCell(buildCell(subArea.getEndNum(), font));
			table.addCell(buildCell(subArea.getSingle().toString(), font));
			table.addCell(buildCell(subArea.getAssistKeyWords(), font));
		}
		// 将表格加入文档
		document.add(table);
	...
	// 组织subAreahighcharts的数据
		@Action(value="report_exportSubAreaHighcharts",results={@Result(name="success",type="json")})
		public String exportSubAreaHighcharts(){
			List<Object[]> list = subAreaService.exportSubAreaHighcharts();
			// 封装数据
			List<ChartData> chartList = new ArrayList<>();
			for(int i=0;i<list.size();i++){
				ChartData chartData = new ChartData();
				BigDecimal data = (BigDecimal)(list.get(i)[0]);
				chartData.setData(data.longValue());
				chartData.setName(list.get(i)[1].toString());
				chartList.add(chartData);
			}
			pushValueStack(chartList);
